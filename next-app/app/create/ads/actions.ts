 "use server";
import {createClient} from "@/lib/supabase/server";import {requireUser,getCurrentWorkspace} from "@/lib/auth";import {runGenerationJob} from "@/lib/ai/jobs";import {buildAdCopyPrompt} from "@/lib/ads/prompt";import type {AdBrief} from "@/lib/ads/types";import {revalidatePath} from "next/cache";
export async function createAdBrief(input:AdBrief){
 ...
 try{
  const result=await runGenerationJob({task:"copy",quality:"fast",prompt:buildAdCopyPrompt(input,product.name),productId:input.productId,metadata:{briefId:brief.id}},1);
