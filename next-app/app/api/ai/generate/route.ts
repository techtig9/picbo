import {NextResponse} from "next/server";import {runGenerationJob} from "@/lib/ai/jobs";import type {GenerationRequest} from "@/lib/ai/types";
export async function POST(req:Request){
 try{
  ...
  const result=await runGenerationJob(request,Number(body.creditCost||1));return NextResponse.json(result);
